...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    const [ isReplying, setRepliying ] = useState(false);
    ...
    const handleReplyClick = () => {
        if(!access_token){
            return toast.error("Necesitas iniciar sesion para comentar")
        }

        setRepliying(preVal => !preVal);
    ...
    const LoadMoreRepliesButton = () => {
        ...
        let button = <button onClick={() => loadReplies({ skip: index - parentIndex, currentIndex: parentIndex })} className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2">Ver mas</button>;
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <div className="flex gap-5 items-center mt-5">

                    {
                        ...
                        className="text-dark-grey p-2 px-3 hover:bg-grey/30 rounded-md flex items-center gap-2">
                            <i className="fi fi-rs-comment-dots"></i>
                            {children.length} Respuestas
                        </button>
                    }

                    <button className="underline" onClick={handleReplyClick}>Contestar</button>
...

                {
                    isReplying ?
                    <div className="mt-8">
                        <CommentField action="reply" index={index} replyingTo={_id} setRepliying={setRepliying}/>
                    </div>
                    :
                    ""
                }
