...
const EditProfile = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if(username.length < 3) {
            return toast.error("El nombre de usuario debe tener al menos 3 caracteres de longitud.");
        }
        if(bio.length > bioLimit) {
            return toast.error(`La biografía no debe sobrepasar los ${bioLimit} caracteres.`);
        }
                ...
                <form ref={editProfileForm}>
                    ...
                    <div className="flex flex-col lg:flex-row items-start py-10 gap-8 lg:gap-10">
                        <div className="max-lg:center mb-5">
                            ...
                            <input type="file" id="uploadImg" accept=".jpeg, .png, .jpg"
                        ...
                        <div className="w-full">
                            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-5">
                                <div>
                                    <InputBox name="fullname" type="text" value={fullname} placeholder="Nombre"
                                        disable={true} icon={"fi-rr-user"} />
                                </div>
                                <div>
                                    <InputBox name="email" type="email" value={email} placeholder="Email"
                                        disable={true} icon={"fi-rr-envelope"} />
                                </div>
                            </div>

                            <InputBox type="text" name="username" value={profile_username}
                                placeholder="Username" icon={"fi-rr-at"} />

                            <p className="text-dark-grey -mt-3">El nombre de usuario se utilizará para identificarte y será visible para todos los usuarios.</p>

                            <textarea name="bio" maxLength={bioLimit} defaultValue={bio}
                                className="input-box h-64 lg:h-40 resize-none leading-7 mt-5 pl-5" placeholder="Biografía"
                                onChange={handleCharacterChange}></textarea>

                            <p className="mt-1 text-dark-grey">{charactersLeft} caracteres restantes</p>

                            <p className="my-6 text-dark-grey">Agrega tus redes sociales</p>

                            <div className="md:grid md:grid-cols-2 gap-x-6">
                                {
                                    Object.keys(social_links).map((key, i) => {
                                        let link = social_links[key];
                                        return <InputBox key={i} name={key} type="text"
                                            value={link} placeholder="https://" icon={"fi " + (key !== 'website'
                                                ? "fi_brands-" + key
                                                : "fi-rr-globe")} />
                                    })
                                }
